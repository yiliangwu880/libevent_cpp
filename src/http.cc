...
bool BaseHttpClient::Request(const char *url, evhttp_cmd_type cmd_type, unsigned int ot_sec, const char *post_data)
{
	...
	////////////////////////////////////
	{
		...
		LB_COND_F(hu.get(), "Parse url failed! ");
		host = evhttp_uri_get_host(hu.get());
		LB_COND_F(host, "Parse host failed! ");
		port = evhttp_uri_get_port(hu.get());
		if (port < 0) port = 80;
		...
		const char *query = evhttp_uri_get_query(hu.get());
		if (query == NULL) {
...
void BaseHttpClient::remote_read_callback(struct evhttp_request* rsp, void* arg)
{
	BaseHttpClient *p = (BaseHttpClient *)arg;
	if (rsp)
	{
		int start_num = rsp->response_code / 100;
		if (start_num != 4 && start_num !=5) //非400,500开头的
		{
			...
			//LOG_DEBUG("replay:");
			int input_len = evbuffer_get_length(rsp->input_buffer);
			char write_addr[MAX_LEN];
			int ret_write_len = evbuffer_remove(rsp->input_buffer, write_addr, sizeof(write_addr));
			if (input_len != ret_write_len)
			{
				LB_ERROR("read data incomplete %d %d", input_len, ret_write_len);
			}
			p->Respond(write_addr);
		}
		else
		{
			p->RespondError(rsp->response_code, rsp->response_code_line);
		}
	}
	else
	{	//超时没响应，或者连接失败
...
{
	LB_ERROR("connection_close_callback");
	BaseHttpClient *p = (BaseHttpClient *)arg;
